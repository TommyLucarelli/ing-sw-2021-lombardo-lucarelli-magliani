...
public class Gui implements UserInterface {
    ...
    private void handleReconnectionUpdate(RequestMsg requestMsg){
        ...
        if(requestMsg.getPayload().has("initial")){
            update = "You are back in the game. Wait for the other players to finish their initial turn";
        }else {
            String currPlayer = requestMsg.getPayload().get("currPlayer").getAsString();
            update = "You are back in the game. " + requestMsg.getPayload().get("currPlayer").getAsString() + " is now playing.";
        }
